...
Q_loc_estimateY = nan(1000); %  position estimate
Q_loc_estimateX= nan(1000); %  position estimate
P_estimate = P;  %covariance estimator
strk_trks = zeros(1,1000);  %counter of how many strikes a track has gotten
nD = size(X{S_frame},1); %initize number of detections
nF =  find(isnan(Q_estimate(1,:))==1,1)-1 ; %initize number of track estimates
ad=0;
%for each frame
c=0;
for t =300:3:400%(length(f_list))-1 
    c=c+1;
...
%     total_tracks=nF
%     newtrk = size(new_trk,2)
%     ad = ad +  newtrk;
%     noftrk= total_tracks - ad 
%     
    ...
    %if a track has a strike greater than 2, delete the tracking. i.e. make
    %it nan first vid = 3
    ...
    %%{
   % clf
   % img = imread(f_list(t).name); imshow(img);
   % hold on;
...
%                st = 49;
%         end
        tmX = Q_loc_estimateX(1:c,Dc);
        tmY = Q_loc_estimateY(1:c,Dc);
...
%save('position_estimates_24oct_800-900.mat', 'Q_loc_estimateX', 'Q_loc_estimateY')
